...
def get_interp(time, lum, t = np.logspace(-8, 4, 500), returns = 'df'):
    """
...
def dtw_dist(lc1, lc2):
    ...
    pointwise_distance = distance.cdist(lc1.reshape(-1,1), lc2.reshape(-1,1))
...
#             cost = abs(lc1[i - 1] - lc2[j - 1]) 
            cost = pointwise_distance[i - 1, j - 1]
...
def sum_class(time, lum, tunits = u.d, lunits = u.erg/u.s, k = 1, verbose = True, skip_zero = True, full = False):
	...
	if verbose:
		...
		if not skip_zero:
			df = pd.DataFrame.from_dict({'class': labels, 'classification probability':out})
		if skip_zero:
			df = pd.DataFrame.from_dict({'class': labels[out > 1], 'classification probability':out[out > 1]})
